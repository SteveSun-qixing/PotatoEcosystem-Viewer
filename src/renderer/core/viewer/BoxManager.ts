 ...
 */
import type { IBoxManager, IEventBus } from '@common/interfaces';
...
export class BoxManager implements IBoxManager {
  ...
  getAvailableLayouts(): Array<{ id: string; name: string }> {
    ...
    return Object.entries(this.currentBox.content.layout_configs).map(([id, config]) => ({
      id,
...
<body>
  <div class="box-container">
    <header class="box-header">
      <h1 class="box-title">${box.metadata.name}</h1>
      <div class="box-meta">
        布局: ${layout} | 卡片数量: ${cardCount}
        ${box.metadata.description ? ` | ${box.metadata.description}` : ''}
      </div>
    </header>
    <main class="box-grid">
      ${cardCount > 0 ? box.structure.cards.map((card, index) => `
        <div class="card-item" data-card-id="${card.id}" data-index="${index}">
          <div class="card-item-title">${card.metadata_cache?.name ?? `卡片 ${index + 1}`}</div>
          <div class="card-item-path">${card.path}</div>
        </div>
      `).join('') : `
